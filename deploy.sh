...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=mohamedthowbick/multi-client:$SHA
kubectl set image deployments/server-deployment server=mohamedthowbick/multi-server:$SHA
